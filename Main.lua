--[[
    SpectrumX UI Library - REMASTERED v2.0
    A modern, fully responsive UI library for Roblox
    GitHub: https://github.com/spectrumxx/SpectrumXUI
    
    CHANGES:
    - Full DPI/Resolution scaling (mobile/PC adaptive)
    - Fixed dropdown positioning (no overlap issues)
    - AssetId support for icons (tabs, header, floating button)
    - Compact Wind UI-inspired design
...
SpectrumX.DPI = {
    ...
    -- Compact sizing (Wind UI style)
    Sizes = {
        Window = Vector2.new(520, 320),        -- Was 600x360
        Header = 42,                           -- Was 55
        Sidebar = 44,                          -- Was 55
        TabButton = 32,                        -- Was 40
        ElementHeight = 36,                    -- Was 45-55
        Toggle = 20,                           -- Was 24
        Input = 24,                            -- Was 28
        Slider = 6,                            -- Was 8
        Padding = 8,                           -- Was 10-12
        TextSmall = 11,                        -- Was 13
        TextNormal = 12,                       -- Was 14
        CornerRadius = 6,                      -- Was 8-12
        FloatingBtn = 44                       -- Was 55
    }
}
...
function SpectrumX:CalculateScale()
    ...
    -- Calculate scale based on screen width (clamped for usability)
    local widthScale = viewport.X / self.DPI.BaseResolution.X
    local heightScale = viewport.Y / self.DPI.BaseResolution.Y
    
    -- Use the smaller dimension to ensure fit, with mobile adjustments
    local rawScale = math.min(widthScale, heightScale)
    
    if self.DPI.IsMobile then
        -- Mobile: slightly larger scale for touch, but not too big
        self.DPI.Scale = math.clamp(rawScale * 1.15, 0.7, 1.1)
    else
        -- PC: compact scale, max 1.0 to keep it sleek
        self.DPI.Scale = math.clamp(rawScale, 0.65, 1.0)
    end
...
-- Theme Configuration (Updated - more contrast, darker)
SpectrumX.Theme = {
    Background = Color3.fromRGB(8, 8, 8),        -- Darker
    Header = Color3.fromRGB(12, 12, 12),       -- Slight contrast
    Sidebar = Color3.fromRGB(10, 10, 10),      -- Darker
    Content = Color3.fromRGB(12, 12, 12),    -- Unified
    Card = Color3.fromRGB(20, 20, 20),       -- Subtle elevation
    Input = Color3.fromRGB(28, 28, 28),      -- Better contrast
    Accent = Color3.fromRGB(255, 45, 45),    -- Slightly adjusted red
    AccentSecondary = Color3.fromRGB(255, 85, 85),
    Text = Color3.fromRGB(245, 245, 245),    -- Slightly softer white
    TextSecondary = Color3.fromRGB(170, 170, 170),
    TextMuted = Color3.fromRGB(120, 120, 120),
    Success = Color3.fromRGB(45, 220, 90),   -- Better green
    Warning = Color3.fromRGB(255, 180, 60),  -- Better orange
    Info = Color3.fromRGB(80, 160, 255),     -- Better blue
    Border = Color3.fromRGB(35, 35, 35),     -- Subtle borders
...
-- Icon/Image Helper (supports AssetId or Text)
function SpectrumX:CreateIcon(parent, config)
    config = config or {}
    local iconType = config.Type or "Text" -- "Text" or "Image"
    ...
    if iconType == "Image" and content then
        -- Support rbxassetid:// format or plain numbers
        ...
        image.ImageLoaded:Connect(function()
            image.ImageColor3 = Color3.new(1, 1, 1) -- Reset tint when loaded
        end)
    else
...
function SpectrumX:CreateWindow(config)
    ...
    self.MainFrame.Position = config.Position or UDim2.new(
        0.5, -windowSize.X/2, 
        0.5, -windowSize.Y/2
    )
    ...
    -- Title Icon (AssetId or Letter support)
    local iconSize = self:Scale(28)
    local iconConfig = {
        Type = config.IconType or "Text", -- "Text" or "Image"
        Content = config.Icon or config.Title and config.Title:sub(1,1) or "S",
        Size = 28,
        Position = UDim2.new(0, self:Scale(12), 0.5, -iconSize/2),
    ...
    title.Position = self:ScaleUDim2(0, 48, 0, 0)
    title.Size = self:ScaleUDim2(0, 250, 1, 0)
    title.Font = Enum.Font.GothamBold -- Slightly lighter
    ...
    closeBtn.Position = self:ScaleUDim2(1, -36, 0.5, -12)
    closeBtn.Size = self:ScaleUDim2(0, 24, 0, 24)
    closeBtn.Font = Enum.Font.GothamBold
    closeBtn.Text = "×" -- Better X
    closeBtn.TextColor3 = self.Theme.TextMuted
    closeBtn.TextSize = self:Scale(20)
    ...
    sidebarLayout.Padding = UDim.new(0, self:Scale(6))
    sidebarLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
    sidebarLayout.Parent = self.Sidebar
    
    local sidebarPadding = Instance.new("UIPadding")
    sidebarPadding.PaddingTop = UDim.new(0, self:Scale(8))
    sidebarPadding.Parent = self.Sidebar
    
    -- Content Area (Adjusted for compact sidebar)
    local contentPadding = self:Scale(8)
    ...
    RunService.Heartbeat:Connect(function()
        if self.ScreenGui and self.ScreenGui.Parent then
            ...
            if currentViewport ~= viewport then
                -- Optional: Auto-resize on viewport change could go here
...
-- Floating Toggle Button (Compact & Icon support)
function SpectrumX:CreateFloatingButton(config)
    ...
    self.FloatBtn.Position = self:ScaleUDim2(0, 10, 0.5, -btnSize/2)
    ...
    floatShadow.Size = UDim2.new(1, self:Scale(10), 1, self:Scale(10))
    ...
    -- Make draggable with better touch handling
...
function SpectrumX:CreateTab(config)
    ...
    -- Support both Icon (text/letter) and IconId (image)
    ...
    local iconConfig = {
        ...
        Content = iconContent,
        Size = 18,
        Position = UDim2.new(0.5, -self:Scale(9), 0.5, -self:Scale(9)),
    ...
    -- Single column layout for compactness (optional 2-column via config)
    local useTwoColumns = config.TwoColumns ~= false -- Default true
...
-- Select Tab (Updated for icon coloring)
...
function SpectrumX:CreateToggle(parent, config)
    ...
    local label = Instance.new("TextLabel")
    label.BackgroundTransparency = 1
    label.Position = self:ScaleUDim2(0, 10, 0, 0)
    ...
    -- Compact switch
    local switchWidth = self:Scale(38)
    ...
    switchBg.Position = UDim2.new(1, -switchWidth - self:Scale(10), 0.5, -switchHeight/2)
    switchBg.Size = UDim2.new(0, switchWidth, 0, switchHeight)
    switchBg.Parent = frame
    self:CreateCorner(switchBg, UDim.new(1, 0))
    
    local circleSize = self:Scale(16)
...
function SpectrumX:CreateSlider(parent, config)
    ...
    local label = Instance.new("TextLabel")
    label.BackgroundTransparency = 1
    label.Position = self:ScaleUDim2(0, 10, 0, 6)
    label.Size = self:ScaleUDim2(0.5, 0, 0, 18)
    label.Font = Enum.Font.GothamSemibold
    label.Text = text
    label.TextColor3 = self.Theme.Text
    label.TextSize = self:Scale(self.DPI.Sizes.TextNormal)
    ...
    valueLabel.Position = self:ScaleUDim2(0.6, 0, 0, 6)
    valueLabel.Size = self:ScaleUDim2(0.35, 0, 0, 18)
    ...
    sliderBg.Position = self:ScaleUDim2(0.03, 0, 0, 32)
...
-- FIXED Dropdown System (No overlap, proper positioning)
function SpectrumX:CreateDropdown(parent, config)
    ...
    label.BackgroundTransparency = 1
    label.Position = self:ScaleUDim2(0, 10, 0, 6)
    label.Size = self:ScaleUDim2(1, -20, 0, 16)
    ...
    dropdownBtn.Position = self:ScaleUDim2(0, 10, 0, 26)
    dropdownBtn.Size = self:ScaleUDim2(1, -20, 0, 22)
    dropdownBtn.Font = Enum.Font.GothamSemibold
    dropdownBtn.Text = "  " .. (default or "Select...")
    dropdownBtn.TextColor3 = self.Theme.TextSecondary
    dropdownBtn.TextSize = self:Scale(10)
    ...
    -- FIXED: Dropdown List - Positioned relative to Screen, not button
    ...
    dropdownList.Size = UDim2.new(0, 0, 0, 0) -- Will be sized dynamically
    dropdownList.ScrollBarThickness = self:Scale(2)
    dropdownList.ScrollBarImageColor3 = self.Theme.Accent
    dropdownList.Visible = false
    dropdownList.ZIndex = 100
    dropdownList.BorderSizePixel = 0
    dropdownList.Parent = self.ScreenGui -- Parent to ScreenGui for proper layering
    ...
    listLayout.Padding = UDim.new(0, self:Scale(3))
    listLayout.Parent = dropdownList
    
    local listPadding = Instance.new("UIPadding")
    listPadding.PaddingTop = UDim.new(0, self:Scale(4))
    listPadding.PaddingBottom = UDim.new(0, self:Scale(4))
    listPadding.PaddingLeft = UDim.new(0, self:Scale(4))
    listPadding.PaddingRight = UDim.new(0, self:Scale(4))
    listPadding.Parent = dropdownList
    
    selectedValue = default
    isOpen = false
    
    -- FIXED: Position calculation that accounts for screen boundaries
    local function positionDropdown()
        ...
        -- Check if would go off bottom of screen
        ...
        if targetX + targetWidth > viewportX then
            targetX = viewportX - targetWidth - self:Scale(10)
        end
    ...
    local function populateList()
        ...
        for _, option in ipairs(options) do
            ...
            optionFrame.BackgroundColor3 = isSelected and Color3.fromRGB(35, 55, 35) or self.Theme.Input
            ...
            if isSelected then
                self:CreateStroke(optionFrame, Color3.fromRGB(70, 220, 70), 1, 0.3)
            end
            ...
            optionBtn.TextColor3 = isSelected and Color3.fromRGB(130, 240, 130) or self.Theme.TextSecondary
            optionBtn.TextSize = self:Scale(10)
            optionBtn.TextXAlignment = Enum.TextXAlignment.Left
            optionBtn.ZIndex = 102
            optionBtn.Parent = optionFrame
            
            local padding = Instance.new("UIPadding")
            padding.PaddingLeft = UDim.new(0, self:Scale(8))
            ...
            optionBtn.MouseEnter:Connect(function()
                if not isSelected then
                    self:Tween(optionFrame, {BackgroundColor3 = Color3.fromRGB(45, 45, 45)}, 0.15)
                end
            end)
...
-- FIXED Multi Dropdown (same positioning fixes)
...
function SpectrumX:CreateCheckbox(parent, config)
    ...
    local frame = Instance.new("Frame")
    frame.BackgroundColor3 = self.Theme.Card
    frame.Size = UDim2.new(1, 0, 0, self:Scale(34))
    frame.Parent = parent
    self:CreateCorner(frame)
    
    local checkSize = self:Scale(16)
    local checkbox = Instance.new("TextButton")
    checkbox.Name = "Checkbox"
    checkbox.BackgroundColor3 = default and self.Theme.Accent or self.Theme.Input
    checkbox.Position = UDim2.new(0, self:Scale(10), 0.5, -checkSize/2)
    ...
    self:CreateCorner(checkbox, UDim.new(0, self:Scale(3)))
    
    local checkboxStroke = self:CreateStroke(checkbox, self.Theme.Accent, 1, 0.5)
    
    local label = Instance.new("TextLabel")
    label.BackgroundTransparency = 1
    label.Position = UDim2.new(0, self:Scale(32), 0, 0)
    label.Size = self:ScaleUDim2(1, -44, 1, 0)
    label.Font = Enum.Font.GothamSemibold
    label.Text = text
    label.TextColor3 = self.Theme.Text
    label.TextSize = self:Scale(self.DPI.Sizes.TextNormal)
...
function SpectrumX:CreateLabel(parent, config)
    ...
    local size = config.Size or UDim2.new(1, 0, 0, self:Scale(26))
    ...
    label.BackgroundTransparency = 1
    label.Position = self:ScaleUDim2(0, 10, 0, 0)
    label.Size = self:ScaleUDim2(1, -20, 1, 0)
    label.Font = Enum.Font.GothamSemibold
    label.Text = text
    label.TextColor3 = color
    label.TextSize = self:Scale(self.DPI.Sizes.TextNormal)
...
function SpectrumX:CreateStatusCard(parent, config)
    ...
    frame.Size = UDim2.new(1, 0, 0, self:Scale(90))
    frame.Active = true
    frame.Parent = parent
    self:CreateCorner(frame)
    ...
    header.Size = UDim2.new(1, 0, 0, self:Scale(26))
    header.Parent = frame
    ...
    statusTitle.TextSize = self:Scale(10)
    statusTitle.TextXAlignment = Enum.TextXAlignment.Left
    statusTitle.Parent = header
    
    local content = Instance.new("Frame")
    content.BackgroundTransparency = 1
    content.Position = self:ScaleUDim2(0, 10, 0, 30)
    content.Size = self:ScaleUDim2(1, -20, 1, -36)
    ...
    infoLabel.TextSize = self:Scale(9)
...
-- FIXED Notification System (Responsive positioning)
function SpectrumX:Notify(config)
    ...
    local notifWidth = math.min(self:Scale(280), workspace.CurrentCamera.ViewportSize.X - self:Scale(40))
    local notifHeight = self:Scale(50)
    ...
    icon.Position = self:ScaleUDim2(0, 10, 0, 0)
    icon.Size = self:ScaleUDim2(0, 24, 1, 0)
    ...
    local label = Instance.new("TextLabel")
    label.BackgroundTransparency = 1
    label.Position = self:ScaleUDim2(0, 40, 0, 0)
    label.Size = self:ScaleUDim2(1, -50, 1, 0)
    label.Font = Enum.Font.GothamSemibold
    label.Text = text
    label.TextColor3 = self.Theme.Text
