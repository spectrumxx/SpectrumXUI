...
function SpectrumX:_closeAllDropdowns()
    if self._dropOverlay then
        self._dropOverlay.Visible = false
    end
    for _, ch in ipairs(self.ScreenGui:GetChildren()) do
        if ch.Name:find("DropdownList_") or ch.Name:find("MultiDropdownList_") then
            ch.Visible = false
            -- força tamanho 0 pra animação não aparecer depois
            ch.Size = UDim2.new(0, ch.AbsoluteSize.X, 0, 0)
        end
    end
    -- Dispara evento pra cada dropdown saber que fechou
    if self._dropCloseCallbacks then
        for _, cb in ipairs(self._dropCloseCallbacks) do
            pcall(cb)
        end
    end
end

function SpectrumX:_registerDropClose(cb)
    if not self._dropCloseCallbacks then
        self._dropCloseCallbacks = {}
    end
    table.insert(self._dropCloseCallbacks, cb)
end

function SpectrumX:_openDropdown(dropList)
    -- Mostra overlay pra capturar clique fora
    if self._dropOverlay then
...
function SpectrumX:CreateWindow(config)
    ...
    -- ── Overlay invisível para detectar clique fora dos dropdowns ──
    -- Fica em ZIndex baixo; quando um dropdown abre, o overlay aparece.
    -- Clicar no overlay fecha todos os dropdowns.
    self._dropOverlay = Instance.new("TextButton")
    self._dropOverlay.Name               = "DropOverlay"
    self._dropOverlay.BackgroundTransparency = 1
    self._dropOverlay.Size               = UDim2.new(1, 0, 1, 0)
    self._dropOverlay.ZIndex             = 1000  -- acima da UI, abaixo das listas (2000)
    self._dropOverlay.Text               = ""
    self._dropOverlay.Visible            = false
    self._dropOverlay.Parent             = self.ScreenGui

    self._dropOverlay.MouseButton1Click:Connect(function()
...
function SpectrumX:CreateDropdown(parent, config)
    ...
    local function closeDropdown()
        ...
        task.wait(0.25)
        dropList.Visible = false
        if self._dropOverlay then self._dropOverlay.Visible = false end
    end

    -- Registra pra fechar quando outro dropdown abrir ou overlay for clicado
    self:_registerDropClose(function()
        if isOpen then
            isOpen = false
            dropList.Visible = false
